...
local checkout — with `go list -m -f '{{.Dir}}' github.com/gotd/td` (currently `v0.160.0`), and
...
Do **not** add `Co-Authored-By:` or `Claude-Session:` trailers to commit messages, whatever
the default attribution guidance says. Commit messages here end with the body.
