...
- [x] Forum topics: `topics` list/get/create/edit/close/reopen/hide/pin/reorder/
      delete/enable/disable, plus `--topic <id>` on every messaging command
      (send, reply, upload, album, poll, schedule, forward, draft, history,
      search, read, pinned, unpin-all, delete-history, context, watch, wait) and
      `topic_id` on message output.
